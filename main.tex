...
Recall Theorem 1.1:   Suppose that $f\in\ZZ[X_1\hdots,X_n]$ is squarefree, satisfies
  ...
  As part of the randomization, the algorithm chooses a set 
  \[
  S := \{1,2,\hdots,\epsilon^{-2}3n^2(2d)^{3n}\}
  \]
  from which to sample from. 
...
\nl Take  $S := \{1,2,\hdots,\epsilon^{-2}3n^2(2d)^{3n}\}$
...
\nl Run \cite[Algorithm 2]{SH} $k \geq -\lg(1-(1+\epsilon)^{(-1/n)})$ times with input (state this in terms of straight line programs?)
...
%
\begin{enumerate}
    \item We defined $S = \{1,2,\hdots,\epsilon^{-2}3n^2(2d)^{3n}\}$ and therefore the height of $a_{i,j} \in S$ is at most 
    \[
