...
Let $f \in \ZZ[X_1,\hdots,X_n]$ have total degree $d$, with $V(f)\subset \C^n$ smooth. In what follows, we fix $i$ in $\{1,\dots,n\}$, and
...
Let $f \in \ZZ[X_1,\hdots,X_n]$ and $i \in \{1,\hdots,n\}$ be as before. We now assume
...
Let $f\in \ZZ[X_1,\hdots,X_n]$, with total degree $d$ and $V(f)\subset \C^n$ smooth, and $i \in \{1,\hdots,n\}$ be fixed throughout this section. We
prove that there exists a non-zero polynomial $\D_{i}$ in $\C[\A]$ of degree at most $5n^2(2d)^{2n}$ such that if $\mA \in \C^{n \times n}$ does not cancel $\D_{i}$,
then $\mA$ is invertible and $f^{\mA}$ satisfies both conditions in
$\textbf{H}_i$.

Consider again the $n\times n$ matrix of indeterminates
$\A=(\A_{j,k})_{1 \le j,k \le n}$ and the field $\C(\A)$,
and define $f^{\A} \in \C(\A)[X_1,\dots,X_n]$.  Since $f$ and $i$ are fixed, to
...
\begin{enumerate}
    \item We defined $S := \{1,2,\hdots,\lceil 3\epsilon^{-1}5n^3(2d)^{2n} \rceil \}$
