...
\begin{theorem}
  ...
  then $\mA$ is invertible and $f^\mA$ satisfies $\textbf{H}_i$ and $\textbf{H}_i^{'}$ for
...
It will on occasion be useful to work with change of variables with
``generic'' entries. For this, we will introduce $n^2$ new indeterminates
$(\frak A_{j,k})_{1\le j,k \le n^2}$, we will let $\K$ be the field
$\C(\frak A_{i,j})$, and use the corresponding
matrix $\A$ over $\K$. For $f$ as above, we will then define the
polynomial $f^\A:=f(\A \Xb) \in \K[X_1,\dots,X_n]$.
...
%% >>>>>>> 3c8b7c51e8e3a65e188ffd3ac7465da21bedb71e
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
\begin{theorem}
There exists a hypersurface $\Delta_1 \subset \C^{n^2}$ of degree at most $nd^{2n}$ with the property that, if $A \in \C^{n^2} - \Delta_1$ then, for each $i \in \{1,\hdots,n\},$ $f^A$ satisfies $\textbf{H}_i(1)$.
\end{theorem}
...
\[
\deg \Delta_1 \leq \sum_{i=1}^n \deg \Xi_i  \leq nd^{2n}.
\] 

In Section~\ref{sec:Hi2}, we will need the following alternative form
of this result: if we consider $f^\A \in \C(\frak
...
\begin{theorem}
  ...
  degree at most $nd^{2n}$ with the property that, if $\bm\alpha \in \C^{n-1} - \Delta_2$, then, for each $i \in \{1,\hdots,n\},$ if $f$ satisfies $\textbf{H}_i$ then $f$ and $\bm \alpha$ satisfy
  $\textbf{H}_i'$.
\end{theorem}
%
...
where we have the following bound on the degree which proves the claim:
\[
\deg \Delta_2 \leq \sum_{i=1}^n \deg \Xi_i \leq n d^{2n}.
\] 
...
prove that there exists a hypersurface $\D_{3,i} \subset \C^{n\times
  n}$ of degree at most \td{} such that if $\mA$ is not in $\D_{3,i}$,
...
\begin{lemma} 
  ...
  $\I^\A$, and such that $\deg(\pjb)\leq (2d)^{n-i+1}.$
\end{lemma} 
\begin{proof}
...
$\C[\A]\setminus\{0\}$ and $ A_{j,\ell},B_j$ in
$\C[\A][X_1,\dots,X_n][T]$, such that
\begin{align*}
\alpha_j = \sum_{\ell=1}^{n-i+1} A_{j,\ell} g_\ell + B_j (1-\pjb T),\ \  g_\ell \in 
\left\{ 
...
%%  Since $P_{j}(X_j) \in\I^{\A}= \langle
%% f^\A,\frac{\partial f^\A}{\partial X_{i+1}},\hdots,\frac{\partial
%%   f^\A}{\partial X_n} \rangle,$ we can write
%% \[
%% P_{j}(X_j) = a_j f^{\A} + \sum_{l=1}^{n-i}b_{j,l} \frac{\partial f^\A}{\partial X_{i+l}}, ~a_j,b_{j,l} \in \mathbb{Q}(\A)[\textbf{X}].
%% \]
%% For each $P_j,$ there exists a Zariski open set $\Gamma_j \subset
%% \C^{n^2}$ such that if $A \in \Gamma_j$, then $A$ cancels none of the
%% denominators of $a_{j}$ or $b_{j,l}$. Define $h_j \in \Q[\A]$ to be
%% the product of the denominators of the coefficients of $a_j$ and
%% $b_{j,l}$. Hence, if $A \in \Q^{n^2}-V(h_j)$ then $A$ cancels none of
%% the denominators of $a_{j}$ or $b_{j,l},$ which are providing the
%% integral dependence relation. Now we define
%% \[
%% \Xi_i := \bigcup_{j=i}^n V(h_j) =V\left(\prod_{j=i}^n h_j \right), 
%% \]
%% and we obtain a bound on the degree of $\Xi_i$ by observing that
...
%% \]
%% is integral. We can obtain bounds for $h_j$ by applying an effective
...
To apply the effective Nullstellensatz theorem, we first need to
determine degree bounds, separately in $\Xb$ and $\A,$ of the
polynomials in the membership relationship. We have
\[
\deg_{\Xb}\left\{ 
f^\A,\frac{\partial f^\A}{\partial X_{i+1}},\hdots,\frac{\partial f^\A}{\partial X_n}
\right\}\leq d,
...
\]
For each $j \in \{i,\hdots,n\},$ by \cite[Theorem 0.5]{EN}, 
%
...
\nl Choose $T \subset \mathbb{Q}$ and $\alpha_{n-1}=(\alpha_1,\hdots,\alpha_{n-1}) \in T^{n-1}$ with $|T| \geq \epsilon^{-1}d^{\td}$\; 
...
%
Recall Theorem 2.1: \textit{  There exists a hypersurface $\D \subset \mathbb{C}^{n^2}$ of degree at most $\td$ such that, if $\mA$ is chosen from $\C^{n^2}-\Delta$ then $\mA$ is invertible and $f^\mA$ satisfies $\textbf{H}_i$ and $\textbf{H}_i^{'}$ for all $i \in \{1,\hdots,n-1\}.$} 
%
\begin{proof}
...
%
\begin{enumerate}
    \item We assume that $S=\{1,2,\hdots,\epsilon^{-1}\td\}$. Therefore, the height of $a_{ij} \in S$ is bounded by $\log \td -\log \epsilon.$
    \item 
