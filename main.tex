...
\]
boolean operations, with $k \geq -\lg(1-2^{(-1/n)})$. The polynomials in the output have degree at most $D^n,$ and height at most
\[
O^{\sim}((s+D)D^n + nD^n).
\]
\end{theorem}
...
%
\textbf{The boolean cost:} The following lists the costs for each step of Algorithm 1:
%
\begin{enumerate}
    ...
    \item 
    After applying \cite[Algorithm 2]{SH} $kn$ times, we obtain the following upper bound on the total boolean cost of the algorithm:
    \[
    O^\sim(kn(Lb+D^{2n}(s+D)(L+n^3D+n^4))),
    \]
