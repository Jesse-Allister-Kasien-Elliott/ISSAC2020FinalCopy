...
which we will consider in either
...
\end{theorem}
\noindent The proof of this theorem occupies the next two sections.
...
% Now, the proof of \cite[Theorem B.3]{TWT} shows that if $\thetab \in
% \C^{\dt}$ is not in $\overline{\pi(Z)}$, then $\bm 0$ is a regular
% value of $\bm\Phi_{\thetab}$.
...
Let us then define 
$$\D_{3,i}:=\D_{2,i} \alpha_i \cdots \alpha_n D_i \cdots D_n,$$
where, for all $j$, $\alpha_j$ is as above and $D_j$ is the leading
coefficient of $\pjb$ with respect to $X_j$. Thus, $\D_{3,i}$ is a
...
To estimate the degree of $\D_{3,i}$, what remains is to give an upper
