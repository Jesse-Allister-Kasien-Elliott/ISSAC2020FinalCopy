...
\copyrightyear{2020}
\acmYear{2020}
\setcopyright{acmlicensed}\acmConference[ISSAC '20]{International Symposium on Symbolic and Algebraic Computation}{July 20--23, 2020}{Kalamata, Greece}
\acmBooktitle{International Symposium on Symbolic and Algebraic Computation (ISSAC '20), July 20--23, 2020, Kalamata, Greece}
\acmPrice{15.00}
\acmDOI{10.1145/3373207.3404058}
\acmISBN{978-1-4503-7100-1/20/07}
...
\begin{abstract}
  ...
  Our paper answers these questions. Our main contribution is a
...
For a zero-dimensional algebraic set $S \subset \C^n$ defined
over $\Q$, a {\em zero-dimensional parameterization}
...
if we apply a generic change of coordinates $\mA$ to $f$, then $W(\pi_i,V)$ is
known to be equidimensional of dimension $(i-1)$ or
...
\item $W(\pi_i,V(f))$ is either empty or in Noether position for
  $\pi_{i-1}$.
\end{enumerate}
When $\textbf{H}_i(1)$ holds, given $\bm \sigma = (\sigma_1,\hdots,\sigma_{i-1})$
...
(X_1-\sigma_1,\dots,X_{i-1}-\sigma_{i-1},f,{\pa f}/{\pa
  X_{i+1}},\dots,{\pa f}/{\pa X_n}),
\]
  the Jacobian matrix of these equations at $\xb$ has full rank $n.$ 

{\em By point (1) above, we know that there are finitely many solutions 
    to these equations.}
...
Note that for a variety $Y \subset \C^n,$ we can define $Y^{\mA}$ as the image of $Y$ by the map $\phi_{\mA} : \xb \mapsto \mA^{-1}\xb.$ Note that $W(\pi_i,V(f^\mA))$ may not equal $W(\pi_i,V(f))^\mA$, as, for instance, their dimensions may vary. 
...
\begin{theorem}\label{theo:Hp}
  Suppose that $f$ satisfies $\textbf{H}_i$. Then for  $i=1,\dots,n$, there exists a non-zero
...
related results appear in the literature; for instance, Lemma 5
in~\cite{JeSa10} or Proposition 4.5
in~\cite{SharpEstimatesForTheEffectiveN} are quantitative Noether
position statements. However, Theorem~\ref{theo:gen} does not follow
...
$$(X_1-\sigma_1,\dots,X_{i-1}-\sigma_{i-1},f,{\pa f}/{\pa
  X_{i+1}},\dots,{\pa f}/{\pa X_n}),$$ the Jacobian matrix of these equations at $\xb$ has full rank $n.$}
 % the ideal
 % $$(X_1-\sigma_1,\dots,X_{i-1}-\sigma_{i-1},f, {\pa f}/{\pa
...
\[
  \left(X_1-S_1,\hdots,X_{i-1}-S_{i-1}, f,\pa f/\pa X_{i+1},\hdots,\pa f/\pa X_n \right).
\]
%
\begin{lemma}
...
\begin{lemma} 
  For $j=1,\dots,n$, there exists a polynomial $P_j$ in
