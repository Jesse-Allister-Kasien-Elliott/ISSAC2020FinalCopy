...
Let us then define $\D_{3,i}:=\D_{2,i} \alpha_i \cdots \alpha_n$.
This is a non-zero polynomial in $\C[\A]$; we will 
estimate its degree below.

\begin{lemma}
  Suppose that $\mA \in \C^{n\times n}$ does not cancel
  $\D_{3,i}$. Then $f^\mA$ satisfies ${\bf H}_i$.
\end{lemma}
\begin{proof}
  By assumption, $\D_{2,i}(\mA)$ is non-zero, so that $\mA$ is invertible
  and satisfies ${\bf H}_i(1)$.
\end{proof}
