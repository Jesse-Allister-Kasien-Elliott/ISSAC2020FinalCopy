...
%
The following is our main algorithm. In step 5, we use  \cite[Algorithm 2]{SH} to solve a square system. This subroutine is randomized. In order to guarantee a higher probability of success, we repeat the calculation $k$ times. This subroutine also requires a straight-line program as input, which we compute from $f^\mA$ and its partial derivatives. 
%
\begin{algorithm}[hbt!]
...
\nl Construct $S := \{1,2,\hdots,3\epsilon^{-1}6n^3(2d)^{2n}\}$ and $T := \{1,2,\hdots,3\epsilon^{-1}nd^{2n}\},$ and randomly choose $\mA \in S^{n^2}$, and  $\bm \sigma \in T^{n-1}$\;
...
\nl Run \cite[Algorithm 2]{SH} $k \geq \lg(3n/\epsilon)$ times with a straight-line program computed from 
$\{X_1-\sigma_1,\hdots,X_{i-1}-\sigma_{i-1},f^{\mA},\frac{\pa f^{\mA}}{\pa X_{i+1}},\hdots,\frac{\pa f^{\mA}}{\pa X_n} \}$\;
