...
\begin{theorem}\label{theo:main}
  ...
  There exists a randomized algorithm that takes $f$
  ...
  As part of the randomization, the algorithm chooses sets 
  \[
  ...
  With probability at least $1-\epsilon$, the output is correct. Otherwise, 
...
by a straight-line program $\Gamma,$ where the length of $\Gamma$ would
be bounded by $d^n$ and would have integer constants of height at most
$b$. Here, $\Gamma$ could be computed in the trivial way, by 
computing the terms and summing them. 
...
\nl Take $S := \{1,2,\hdots,3\epsilon^{-1}6n^3(2d)^{2n}\}$ and $T := \{1,2,\hdots,3\epsilon^{-1}nd^{2n}\},$ and randomly choose $A \in S^{n^2}$, and  $\sigma \in T^{n-1}$\;
\nl Apply the change of variables to obtain $f(A x)=f^A(x)$\;
...
$\{Y_1-\sigma_1,\hdots,Y_{i-1}-\sigma_{i-1},f^A,\frac{\pa f^A}{\pa X_{i+1}},\hdots,\frac{\pa f^A}{\pa X_n} \}$\;
