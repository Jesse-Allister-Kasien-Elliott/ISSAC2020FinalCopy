...
\]
boolean operations, with $k \geq -\lg(1-2^{(-1/n)})$. 
\end{theorem}
%
...
\[
O^{\sim}(k(Lb+D^{2n}(s+D)(L+n^3D+n^4)))
\]
boolean operations, with $k \geq -\lg(1-2^{(-1/n)})$. }
%
...
\nl Run the symbolic homotopy algorithm from \cite{SH} $k \geq -\lg(1-2^{(-1/n)})$ times with input 
\[
