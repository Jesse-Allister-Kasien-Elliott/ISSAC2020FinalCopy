...
%
The following is our main algorithm. In step 5, we use  \cite[Algorithm 2]{SH} to solve a square system, where this subroutine is randomized. In order to guarantee a higher probability of success, we repeat the calculation $k$ times. This subroutine requires the input to be in the form of a straight-line program, which we compute from $f^\mA$ and its partial derivatives. 
%
\begin{algorithm}[hbt!]
\KwIn{$f \in \Q[X_1,\hdots,X_n]$ of degree $d$ and $0 < \epsilon < 1$}
...
\nl Construct $S := \{1,2,\hdots,\lceil(3\epsilon^{-1})6n^3(2d)^{2n}\}\rceil$ and $T := \{1,2,\hdots,\lceil (3\epsilon^{-1})nd^{2n}\rceil\},$ and randomly choose $\mA \in S^{n^2}$, and  $\bm \sigma \in T^{n-1}$\;
\nl Apply the change of variables to obtain $f(\mA x)=f^{\mA}(x)$\;
\nl Use \cite{partialDerComputation} to compute the partial derivatives $\frac{\pa f^{\mA}}{\pa X_{i+1}},\hdots,\frac{\pa f^{\mA}}{\pa X_n}$\;
    \caption{{Main Algorithm} \label{}}
\nl \For{$i\gets1$ \KwTo $n$}{
\nl Prepare a straight-line program $\Gamma$ that computes the system of polynomials
$\{X_1-\sigma_1,\hdots,X_{i-1}-\sigma_{i-1},f^{\mA},\frac{\pa f^{\mA}}{\pa X_{i+1}},\hdots,\frac{\pa f^{\mA}}{\pa X_n} \}$ and run \cite[Algorithm 2]{SH}, with input $\Gamma,$ and with $ k \geq \lg(3n/\epsilon)$ repetitions\; 
...
\begin{enumerate}
    ...
    \]
    \item  We can prepare a straight-line program $\Gamma$ for $f^{\mA}$ with length at most $d^n$, and 
    by \cite[Lemma 1.2(1)(c)]{SharpEstimatesForTheEffectiveN}, height at most 
    \[
...
%
\par \noindent 
\textbf{Probability of success:} Let $\Delta_i \in \C[\A]$ be the polynomials from Theorem 2.1. Denote by $\Delta := \prod_{i=1}^n \D_i,$ and note that 
\begin{align}
    \deg \Delta \leq \sum_{i=1}^n \deg \Delta_i \leq 6n^3(2d)^{2n},
\end{align}
where then, if $\mA \in \C^{n \times n}$ does not cancel $\Delta,$ then  $\mA$ is invertible and $f^\mA$ satisfies $\textbf{H}_i$ for all $i \in \{1,\hdots,n\}.$ Now, assuming that $\mA$ is such a matrix, let $\Xi_i\in \C[\A]$ be the polynomials from Theorem 2.2 applied to $f^{\mA}.$ Denote by $\Xi := \prod_{i=1}^n \Xi_i,$ and note that 
\begin{align}
    \deg \Xi \leq \sum_{i=1}^n \deg \Xi_i \leq nd^{2n},
\end{align}
where then, if $\bm \sigma \in \C^{i-1}$ does not cancel $\Xi$ then $f^{\mA}$ and $\bm \sigma$ satisfy $\textbf{H}_i^{'}$ for all $i \in \{1,\hdots,n\}.$ 
Now, by construction of 
\[
S := \{1,2,\hdots,3\epsilon^{-1}6n^3(2d)^{2n}\}
\] 
and 
\[
T := \{1,2,\hdots,3\epsilon^{-1}nd^{2n}\},
\] 
where $\mA \in S^{n^2}$ and  $\bm \sigma \in T^{n-1}$ are randomly chosen, we have 
%
\[
\pr[\mA \in \Delta] \leq  \frac{\deg\Delta}{|S|} = \epsilon/3
\]
and
\[
\pr[\bm \sigma \in \Xi] \leq \deg \frac{\deg\Xi}{|S|} = \epsilon/3.
\]
%
...
\begin{align*}
 \pr[\mA \not\in \Delta] \times \pr[\bm \sigma \not \in \Xi ~|~\mA \not\in \Delta] \times \pr[\mathscr{E}~|~\mA \not\in \Delta \textrm{ and } \bm\sigma \not \in \Xi].
\end{align*}
%
...
\begin{align*}
\pr[\textrm{success}] &\geq  (1- \epsilon/3)(1- \epsilon/3)\pr[\mathscr{E}~|~\mA \not\in \Delta \textrm{ and } \bm \sigma \not \in \Xi]\\
