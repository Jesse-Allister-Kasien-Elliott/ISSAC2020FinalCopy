...
\nl Construct $S := \{1,2,\hdots,3\epsilon^{-1}6n^3(2d)^{2n}\}$ and $T :=
\{1,2,\hdots,3\epsilon^{-1}nd^{2n}\},$ and randomly choose $\mA \in
...
\nl \For{$i\gets1$ \KwTo $n$}{
 ...
 equations
  ...
  zero-dimensional parameterization returned in step 5 \; 
} 
...
\begin{enumerate}
    \item We defined $S = \{1,2,\hdots,2\epsilon^{-1}3n^2(2d)^{3n}\}$
      and therefore the height of any $a_{i,j} \in S$ is at most
    \[
    \log 1/{\epsilon} + \log(3n^2(2d)^{2n}) \in O^{\sim}(n\log d + \log 1/{\epsilon}).
    \]
    \item As a result, by \cite[Lemma 1.2(1)(c)]{SharpEstimatesForTheEffectiveN}, all coefficients in 
      $f^\mA$ have height $ O^{\sim}(b + d\log 1 / \epsilon + dn)$.
    \item 
...
and
\[
\pr[\Xi(\bm \sigma)=0] \leq \deg \frac{\deg\Xi}{|T|} = \epsilon/3.
\]
%
...
\end{align*}
This finishes the proof our main theorem.
