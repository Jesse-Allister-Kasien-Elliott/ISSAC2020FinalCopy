...
In general, we cannot say much about the geometric properties of
$W(\pi_i,V)$, but if we apply a generic change of coordinates $\bm A$,
...
in~\cite{Mumford76,Shafarevich77,Eisenbud96}; we will recall key
...
change of coordinates $\bm A$ guarantee success. This is done by
...
randomized algorithms in real algebraic geometry. An immediate
...
%
Consider $A \in \C^{n^2}, f \in \C[X_1,\hdots,X_n], \xb \in \mathbb{C}^n,$ and $V \subset \C^n$. We denote by 
...
\begin{enumerate}
    ...
    \item $V(f) \cap \R^n$ is smooth.
\end{enumerate}
%
An equidimensional algebraic set $X$ of dimension $r$ is in \textit{Noether position} for $\pi_r$ when the extension 
\[
\C[X_1,\hdots,X_r] \rightarrow \C[X_1,\hdots,X_n]/I(V)
\] 
is injective and integral.  Then, for any $\xb \in \C^r,$ the fiber 
$V \cap \pi_r^{-1}(\xb)$ has dimension zero. For $i\in\{1,\hdots,n\}$, and for a randomly chosen point $p_{i-1} = (\alpha_1,\hdots,\alpha_{i-1}) \in \Q^{i-1}$, we say that $f$ satisfies $\textbf{H}_i^{'}$ if 
%
\begin{enumerate}
\item $\I(\pi_i,V)$ is radical;
\item either $W(\pi_i,V)$ is empty or $(i-1)$-equidimensional;
\item $W(\pi_i,V)$ is smooth;
\item $\langle X_1-\alpha_1,\hdots,X_{i-1}-\alpha_{i-1}, f,  \frac{\pa f}{\pa X_{i+1}},\hdots,\frac{\pa f}{\pa X_n}\rangle $ is radical; 
\item $V(X_1-\alpha_1,\hdots,X_{i-1}-\alpha_{i-1}, f,  \frac{\pa f}{\pa X_{i+1}},\hdots,\frac{\pa f}{\pa X_n})$ is finite;
\item $V(X_1-\alpha_1,\hdots,X_{i-1}-\alpha_{i-1}, f,  \frac{\pa f}{\pa X_{i+1}},\hdots,\frac{\pa f}{\pa X_n})$ is smooth;
...
%
Let $p_{n-1} = (x_1,\hdots,x_{n-1}) \in \Q^{n-1}$ be a randomly chosen point. For $i \in \{1,\hdots,n\}$, we want to show that the Jacobian of the system of polynomials
\[
X_1(\xb)-x_1,\hdots,X_{i-1}(\xb)-x_{i-1}, f(\xb),  \frac{\pa f(\xb)}{\pa X_{i+1}},\hdots,\frac{\pa f(\xb)}{\pa X_n}
\]
